...
class passenger extends Client {
    ...
    async fetchCommands() {
        let dirs = fs.readdirSync("./passenger/Commands", { encoding: "utf8" });
        this.logger.log(`${dirs.length} category in client loaded.`, "category");
        dirs.forEach(dir => {
            let files = fs.readdirSync(`./passenger/Commands/${dir}`, { encoding: "utf8" }).filter(file => file.endsWith(".js"));
            this.logger.log(`${files.length} commands loaded in ${dir} category.`, "load");
            files.forEach(file => {
                let referans = require(`./passenger/Commands/${dir}/${file}`);
...
module.exports = { passenger, Mongo };